...
theta2   = (theta2 + np.pi) % (2*np.pi) - np.pi
    ...
    # Setup the the plot
fig, ax = plt.subplots()
ax.set_xlim(-1.1 * (length_2+length_1), 1.1 * (length_2+length_1) )
ax.set_ylim(-1.1 * (length_2+length_1), 1.1 * (length_2+length_1) )
...
if( False ):
    ...
    length_1_label  = ax.text(1.03, 0.79, f"$l_1 = {length_1:.2f}m $", transform=ax.transAxes, fontsize=12, ha='left', va="top")
    length_2_label  = ax.text(1.03, 0.73, f"$l_2 = {length_2:.2f}m $", transform=ax.transAxes, fontsize=12, ha='left', va="top")
...
def update(frame):
    ...
    return line1, bulk1, line2, bulk2, theta1_label, theta2_label, time_label, energy_label 
